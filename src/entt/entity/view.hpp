...
class RuntimeView {
    ...
    class Iterator {
        ...
        Iterator(underlying_iterator_type begin, underlying_iterator_type end, const view_type **first, const view_type **last, extent_type extent)
            : begin{begin},
              end{end},
              first{first},
              last{last},
    ...
    private:
        underlying_iterator_type begin;
        underlying_iterator_type end;
        const view_type **first;
        const view_type **last;
        extent_type extent;
    };

    RuntimeView(pattern_type pools) ENTT_NOEXCEPT
        : pools{std::move(pools)}
    {}

    void promote() const ENTT_NOEXCEPT {
    ...
    extent_type min() const ENTT_NOEXCEPT {
        const auto it = std::min_element(pools.cbegin(), pools.cend(), [](const auto *lhs, const auto &rhs) {
            return !lhs || (rhs && (lhs->extent() < rhs->extent()));
        });

        return (*it) ? (*it)->extent() : extent_type{};
    }
...
public:
     ...
     */
    bool empty() const ENTT_NOEXCEPT {
        return std::any_of(pools.cbegin(), pools.cend(), [](const auto *view) {
    ...
    const_iterator_type cbegin() const ENTT_NOEXCEPT {
        ...
        return const_iterator_type{
            pools.front() ? pools.front()->cbegin() : underlying_iterator_type{},
            pools.front() ? pools.front()->cend() : underlying_iterator_type{},
            pools.data() + 1,
    ...
    bool contains(const entity_type entity) const ENTT_NOEXCEPT {
        const auto sz = size_type(entity & traits_type::entity_mask);
        const auto extent = min();

        return sz < extent && std::all_of(pools.cbegin(), pools.cend(), [entity](const auto *view) {
            return view && view->fast(entity) && view->data()[view->get(entity)] == entity;
        });
    }
    ...
    template<typename Func>
    void each(Func func) const {
        std::for_each(cbegin(), cend(), [func = std::move(func)](const auto entity) {
            func(entity);
        });
    }

private:
    mutable pattern_type pools;
};
